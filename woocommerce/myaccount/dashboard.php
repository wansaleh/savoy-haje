...
<div class="nm-MyAccount-dashboard">
  <h1><?php esc_html_e( 'Dashboard', 'savoy-haje' ); ?></h1>
  <p>
<?php
  echo sprintf( esc_attr__( 'Hello %s%s%s (not %2$s? %sSign out%s)', 'savoy-haje' ), '<strong>', esc_html( $current_user->display_name ), '</strong>', '<a href="' . esc_url( wc_get_endpoint_url( 'customer-logout', '', wc_get_page_permalink( 'myaccount' ) ) ) . '">', '</a>' );
?>
  </p>
